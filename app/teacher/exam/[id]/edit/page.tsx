...
import type { Exam, ExamDay } from "@/types/exam";
import type { Question, Alternative } from "@/types/question";
...
export default function EditExamPage() {
  ...
  const subjects = exam ? daySubjects[exam.day] : [];

  const saveQuestionToFirestore = useCallback(
    async (q: QuestionDraft, order: number): Promise<string> => {
      const questionRef = q.firestoreId
        ? doc(db, "questions", q.firestoreId)
  ...
  async function handleAddOrUpdateQuestion() {
    setError("");
    ...
    try {
      if (editingIndex !== null) {
        ...
        const firestoreId = await saveQuestionToFirestore(q, order);
        setQuestions((prev) => [...prev, { ...q, firestoreId }]);
  ...
  async function handleDeleteQuestion(index: number) {
    ...
    try {
      if (q.firestoreId) {
        await deleteDoc(doc(db, "questions", q.firestoreId));
      }
                    ...
                    <div className="flex flex-col items-center gap-2 text-center">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-500">
                          PNG, JPG ou WEBP (Max. 500KB recomendado)
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {currentQuestion.alternatives.map((alt, i) => (
                  <div key={alt.label} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-1 flex-1 rounded-lg border border-green-200 px-3 py-2 text-sm text-green-900 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 md:w-full"
                    />
                  </div>
                ))}
              </div>
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between">
          <Button variant="outlined" onClick={() => setStep("questions")}>
            Voltar
          </Button>
